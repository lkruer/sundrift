...
export default function (THREE) {
  ...
  // walls: dark backing board and eleven boards with 0.02 m gaps, back and both sides
  const planks = (w, x, z, ry) => {
    const gr = new THREE.Group(); gr.position.set(x, 0, z); gr.rotation.y = ry; g.add(gr);
    const b = new THREE.Mesh(new THREE.BoxGeometry(w, 2.2, 0.02), dark); b.position.set(0, 1.2, -0.02); gr.add(b);
    for (let k = 0; k < 11; k++) { const m = new THREE.Mesh(new THREE.BoxGeometry(w, 0.18, 0.03), timber); m.position.set(0, 0.19 + k * 0.2, 0.005); gr.add(m); }
  };
  planks(2.4, 0, -0.52, 0);
