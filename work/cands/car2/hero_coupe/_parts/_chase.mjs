// Render the car from the chase camera: 4 m back, 1.5 m up, three-quarter rear, plus a dead-rear low view.
...
renderer.setSize(W * 2, H, false); renderer.outputColorSpace = THREE.SRGBColorSpace; document.body.appendChild(renderer.domElement);
...
const views = [[1.7, 1.5, -3.65, 0, 0.6, -0.6], [0.0, 0.9, -4.2, 0, 0.55, -1.0]];
views.forEach((v, i) => {
  const cam = new THREE.PerspectiveCamera(40, W / H, 0.1, 100); cam.position.set(v[0], v[1], v[2]); cam.lookAt(v[3], v[4], v[5]);
...
const page = await browser.newPage(); await page.setViewport({ width: 1800, height: 600 });
