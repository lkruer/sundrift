...
export default function (THREE) {
  ...
  add(body, loft([bumper(1.56, 0.81, 0.64), bumper(1.95, 0.80, 0.605), bumper(2.145, 0.72, 0.60)],
                 { creaseRows: [1, 11, 4, 8], capFront: 0, capBack: 0 }), PAINT);
  add(body, loft([bumper(-1.56, 0.81, 0.74), bumper(-2.0, 0.80, 0.77), bumper(-2.21, 0.72, 0.78)],
                 { creaseRows: [1, 11, 4, 8], capFront: 0, capBack: 0 }), PAINT);
  ...
  for (const s of [-1, 1]) {
    ...
    for (const az of [1.275, -1.225]) {
      ...
      // the front tub's end walls stop under the fender shoulder, which tucks in to the
      // bonnet edge at y 0.71 there; a 0.72 wall poked its corner through the paint
