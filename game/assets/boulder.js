...
export default function (THREE) {
  ...
  // --- moss cap: 0.9 x 0.3 x 0.8 m, sunk into the crown. The hash leaves the highest
  //     vertex off to one side, and a cap centred on it hung over the edge in the first
  //     render; so it sits halfway between the plan centre and the high point, at the
  //     crown height measured there, with its centre 0.08 below that so the thin rim
  //     is buried, the high vertex stays under the moss, and about 0.1 m of moss shows.
  const top = new THREE.Vector3(0, -1, 0);
  for (let i = 0; i < rp.count; i++) if (rp.getY(i) > top.y) top.set(rp.getX(i), rp.getY(i), rp.getZ(i));
  const px = top.x * 0.5, pz = top.z * 0.5;
  let ys = 0;
  for (let i = 0; i < rp.count; i++) {
    const dx = rp.getX(i) - px, dz = rp.getZ(i) - pz;
    if (dx * dx + dz * dz < 0.16 && rp.getY(i) > ys) ys = rp.getY(i);
  }
  ...
  capMesh.position.set(px, ys - 0.08, pz);
